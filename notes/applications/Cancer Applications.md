---
title: Cancer Applications
aliases:
  - SCP in Cancer Research
  - Tumor Proteomics
tags:
  - applications
  - cancer
  - heterogeneity
  - clinical
category: application
---

# Cancer Applications

**Single-Cell Proteomics in Cancer Research**

## Overview

Single-cell proteomics provides unprecedented resolution for understanding tumor heterogeneity, drug resistance mechanisms, and immune microenvironment dynamics.

## Key Applications

### Tumor Heterogeneity
- Identify rare cell populations
- Map clonal evolution
- Detect drug-resistant subclones

### Drug Response
- Single-cell pharmacoproteomics
- Resistance mechanism discovery
- Biomarker identification

### Immune Microenvironment
- T cell exhaustion states
- Macrophage polarization
- Immune checkpoint expression

## Cancer Types Studied

| Cancer | Key Findings | Reference |
|--------|--------------|-----------|
| Triple-Negative Breast Cancer | Cellular diversity, resistance | Nalla et al. 2025 |
| Melanoma | Immune evasion | Multiple studies |
| Leukemia | Differentiation hierarchies | Specht et al. 2021 |
| Lung Cancer | Drug resistance | Emerging |

## Advantages Over Bulk

1. **Detect rare cells**: <1% of population
2. **Avoid averaging**: True distributions
3. **Track dynamics**: Pseudotime analysis
4. **Protein-level**: Post-transcriptional regulation

## Integration Opportunities

- **Multi-omics**: SCP + scRNA-seq
- **Spatial**: Location-aware proteomics
- **Temporal**: Time-course studies

## Challenges

- Limited throughput for clinical samples
- Need for fresh tissue
- Complex data interpretation
- Statistical power considerations

## Key Publications

> [!cite] Key References
> - Nalla et al. (2025) *Proteomics Clin Appl* - TNBC review
> - Specht et al. (2021) *Genome Biology* - Macrophage heterogeneity
>   DOI: [10.1186/s13059-021-02267-5](https://doi.org/10.1186/s13059-021-02267-5)
> -  Sadiku, P., Brenes, A.J., Mayer, R.L. _et al._ Single cell proteomic analysis defines discrete neutrophil functional states in human glioblastoma. _Nat Commun_ (2025). [https://doi.org/10.1038/s41467-025-67367-3](https://doi.org/10.1038/s41467-025-67367-3)





## Future Directions

- Clinical biomarker discovery
- Patient stratification
- Drug combination optimization
- Minimal residual disease detection

## Related Topics

- [[Cell Differentiation]] - Developmental hierarchies
- [[SCoPE2]] - Primary method used
- [[plexDIA]] - High-throughput alternative
